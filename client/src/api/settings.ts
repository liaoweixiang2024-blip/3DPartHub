...
function getResponseMessage(value: unknown): string | undefined {
  ...
  return [data.detail, data.message, data.data?.detail, data.data?.message]
...
export async function pollVerifyBackupProgress(
  ...
  return new Promise((resolve, reject) => {
    ...
    const poll = setInterval(async () => {
      try {
        const res = await client.get(`/settings/backup/verify-progress/${jobId}`, { timeout: 15000 });
        consecutiveErrors = 0;
