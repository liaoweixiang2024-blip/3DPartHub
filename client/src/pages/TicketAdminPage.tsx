...
function useTicketAdminData() {
  ...
  const { visibleItems: visibleTickets, hasMore, loadMore } = useVisibleItems(filtered, 60, `${filter}:${tickets.length}`);
  const counts: Record<string, number> = { all: tickets.length };
  for (const status of statuses.filter((s) => s.tab)) counts[status.value] = tickets.filter((t) => t.status === status.value).length;
...
function EmptyTickets() {
  return (
    <div className="text-center py-20">
      <Icon name="inbox" size={48} className="mx-auto mb-3 text-on-surface-variant/20" />
...
function DesktopContent() {
  ...
  return (
    <AdminManagementPage
      ...
      actions={(
          <button onClick={loadTickets} className="inline-flex items-center gap-1.5 rounded-md border border-outline-variant/20 bg-surface-container-low px-3 py-2 text-xs font-medium text-on-surface transition-colors hover:bg-surface-container">
            <Icon name="refresh" size={16} />刷新
          </button>
      )}
      toolbar={(
        <StatusTabs tabs={statusTabs} active={filter} counts={counts} onChange={setFilter} />
      )}
    >
        ...
        <div className="bg-surface-container-low rounded-lg border border-outline-variant/10 overflow-auto max-h-[calc(100vh-260px)]">
          ...
          {visibleTickets.map((ticket) => {
            ...
            return (
              ...
              >
                <span className={`inline-flex w-fit items-center text-xs px-2 py-0.5 rounded-md font-bold ${info.color || ""} ${info.bg || ""}`}>
                  {info.label}
                </span>
                <span className="text-xs text-on-surface-variant truncate">{classificationMap.get(ticket.classification) || ticket.classification}</span>
                ...
                <span className="text-xs text-on-surface-variant">{new Date(ticket.createdAt).toLocaleDateString("zh-CN")}</span>
                <div className="flex flex-wrap items-center justify-end gap-1">
                    <button onClick={() => navigate(`/admin/tickets/${ticket.id}`)} className="text-xs text-primary-container hover:underline">详情</button>
                    {ticket.status !== "in_progress" && ticket.status !== "resolved" && (
                      <button onClick={() => handleStatusChange(ticket.id, "in_progress")} className="text-xs text-blue-500 hover:underline">处理</button>
                    )}
                    {ticket.status === "in_progress" && (
                      <button onClick={() => handleStatusChange(ticket.id, "resolved")} className="text-xs text-green-500 hover:underline">解决</button>
                    )}
                    {ticket.status !== "closed" && (
                      <button onClick={() => handleStatusChange(ticket.id, "closed")} className="text-xs text-on-surface-variant hover:underline">关闭</button>
                    )}
                    {ticket.status === "closed" && (
                      <button onClick={() => handleStatusChange(ticket.id, "open")} className="text-xs text-primary-container hover:underline">重开</button>
                    )}
                </div>
              </div>
            );
...
function MobileContent() {
  ...
  return (
    <AdminManagementPage
      ...
      description="管理用户提交的模型需求工单"
      actions={(
        <button onClick={loadTickets} className="inline-flex items-center gap-1.5 rounded-md border border-outline-variant/20 bg-surface-container-low px-3 py-2 text-xs font-medium text-on-surface">
          <Icon name="refresh" size={14} />刷新
        </button>
      )}
      toolbar={(
        <StatusTabs tabs={statusTabs} active={filter} counts={counts} onChange={setFilter} compact />
      )}
    >
      <div key={filter} className="admin-tab-panel">
      {loading ? (
        ...
        <div className="flex flex-col gap-2.5">
          {visibleTickets.map((ticket) => {
            const info = statusInfo(statuses, ticket.status);
            return (
              <div key={ticket.id} className="bg-surface-container-high rounded-lg p-3.5 active:bg-surface-container-highest transition-colors">
                <div onClick={() => navigate(`/admin/tickets/${ticket.id}`)} className="cursor-pointer">
                  <div className="flex flex-wrap items-center gap-2 mb-2">
                    ...
                    <span className="text-[10px] text-on-surface-variant">{new Date(ticket.createdAt).toLocaleDateString("zh-CN")}</span>
                  </div>
                  ...
                  {ticket.basePart && <p className="mt-1 text-[11px] text-on-surface-variant break-all">基准零件：{ticket.basePart}</p>}
                </div>
                <div className="mt-3 flex flex-wrap items-center gap-1 border-t border-outline-variant/10 pt-3">
                  {ticket.status !== "in_progress" && ticket.status !== "resolved" && (
                    <button onClick={() => handleStatusChange(ticket.id, "in_progress")} className="px-2.5 py-1 text-xs text-blue-500 border border-blue-500/30 rounded-sm">开始处理</button>
                  )}
                  {ticket.status === "in_progress" && (
                    <button onClick={() => handleStatusChange(ticket.id, "resolved")} className="px-2.5 py-1 text-xs text-green-500 border border-green-500/30 rounded-sm">标记解决</button>
                  )}
                  {ticket.status !== "closed" ? (
                    <button onClick={() => handleStatusChange(ticket.id, "closed")} className="px-2.5 py-1 text-xs text-on-surface-variant border border-outline-variant/20 rounded-sm">关闭</button>
                  ) : (
                    <button onClick={() => handleStatusChange(ticket.id, "open")} className="px-2.5 py-1 text-xs text-primary-container border border-primary-container/30 rounded-sm">重新打开</button>
                  )}
                </div>
              </div>
            );
