...
function DesktopContent() {
  ...
  const { visibleItems: visibleInquiries, hasMore, loadMore } = useVisibleItems(inquiries, 60, String(inquiries.length));

  return (
    <AdminManagementPage
      title="我的询价"
      description="查看你提交过的选型询价和处理进度"
      actions={inquiries.length > 0 ? (
          <Link to="/selection" className="flex items-center gap-2 rounded-lg bg-primary-container px-5 py-2.5 text-sm font-medium text-on-primary hover:opacity-90">
    ...
    >
      ...
      {isLoading ? (
        ...
        <AdminEmptyState
          ...
          action={<Link to="/selection" className="rounded-md bg-primary-container px-5 py-2.5 text-sm font-semibold text-on-primary transition-opacity hover:opacity-90">去选型</Link>}
        />
      ) : (
        <div className="bg-surface-container-low rounded-lg border border-outline-variant/10 overflow-auto">
          ...
          {visibleInquiries.map((inq) => (
            <div key={inq.id} className="grid grid-cols-[80px_1fr_120px_140px] gap-4 px-6 py-4 border-b border-outline-variant/5 hover:bg-surface-container-high/50 transition-colors items-center">
...
function MobileContent() {
  ...
  const statuses = getBusinessConfig(settings).inquiryStatuses;
  const navigate = useNavigate();
  const { visibleItems: visibleInquiries, hasMore, loadMore } = useVisibleItems(inquiries, 40, String(inquiries.length));
  ...
  return (
    <AdminManagementPage
      ...
      description="查看你提交过的选型询价和处理进度"
      actions={inquiries.length > 0 ? (
        <Link to="/selection" className="inline-flex h-9 shrink-0 items-center gap-1.5 rounded-lg bg-primary-container px-3 text-xs font-medium text-on-primary">
