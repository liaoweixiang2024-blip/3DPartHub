...
const GROUPS: SettingGroup[] = [
  ...
  },
  {
    ...
    icon: 'palette',
    items: [
      { key: 'site_title', label: '网站名称', desc: '显示在导航栏和登录页的站点名称（如：零件库）', type: 'text' },
      { key: 'site_browser_title', label: '浏览器标题', desc: '显示在浏览器标签页的标题（如：零件库 — 3D模型管理平台），留空则使用网站名称', type: 'text' },
      { key: 'site_logo', label: '站点 Logo', desc: '上传自定义 Logo 图片，建议尺寸 64×64，PNG/SVG 格式透明背景', type: 'image' },
      ...
      { key: 'site_favicon', label: 'Favicon 图标', desc: '浏览器标签页图标，建议 32×32 或 64×64，支持 ICO/PNG/SVG 格式', type: 'image' },
