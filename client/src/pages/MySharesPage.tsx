...
function ShareTypeBadge({ type }: { type?: ShareLink["type"] }) {
  ...
  return (
    <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-medium ${isSelection ? "bg-blue-500/10 text-blue-400" : "bg-primary-container/12 text-primary-container"}`}>
      {isSelection ? "选型" : "模型"}
    </span>
  );
}

function ShareRow({ item, deleting, selectMode, selected, onToggleSelect, onCopy, onDeleteStart, onDeleteCancel, onDeleteConfirm }: {
  item: ShareLink;
  deleting: boolean;
  selectMode: boolean;
  selected: boolean;
  ...
  return (
    <div className={`group flex min-w-0 items-start gap-2 border-b border-outline-variant/10 px-3 py-2.5 transition-colors last:border-b-0 md:items-center md:px-4 md:py-3 ${selected ? "bg-primary-container/5" : ""}`}>
      {selectMode ? (
        <button
          type="button"
          ...
          className={`mt-1 flex h-6 w-6 shrink-0 items-center justify-center rounded-md border transition-colors md:mt-0 md:self-center ${
            selected ? "border-primary-container bg-primary-container text-on-primary" : "border-outline-variant/35 text-transparent hover:border-primary-container/60"
          }`}
      ...
      >
        <div className="flex min-w-0 items-center gap-1.5 md:gap-2">
          ...
          <h3 className="min-w-0 flex-1 truncate text-sm font-semibold leading-5 text-on-surface group-hover:text-primary-container">{title}</h3>
        </div>
        <div className="mt-1 flex min-w-0 items-center gap-x-2 overflow-hidden text-[11px] text-on-surface-variant md:flex-wrap md:gap-x-3 md:gap-y-1 md:overflow-visible">
          <span className="inline-flex shrink-0 items-center gap-1"><Icon name="visibility" size={12} />{item.viewCount}</span>
          {item.type !== "selection" ? (
            <span className="inline-flex shrink-0 items-center gap-1"><Icon name="download" size={12} />{item.downloadCount}{item.downloadLimit > 0 ? `/${item.downloadLimit}` : ""}</span>
          ) : null}
          {item.hasPassword ? <span className="shrink-0">有密码</span> : null}
          <span className="shrink-0">{formatDate(item.createdAt)}</span>
          {expired ? <span className="shrink-0 text-error">已过期</span> : item.expiresAt ? <span className="hidden shrink-0 md:inline">有效至 {formatDate(item.expiresAt)}</span> : <span className="hidden shrink-0 md:inline">永久有效</span>}
        </div>
      </Link>
  ...
  const toolbar = shares.length ? (
    <div className="flex min-w-0 flex-col gap-2 md:flex-row md:items-center">
      <div className="relative min-w-0 flex-1 md:max-w-sm">
        <Icon name="search" size={16} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-on-surface-variant/60" />
        <input
          value={search}
      ...
      >
        {shares.length === 0 ? (
          <AdminEmptyState
            ...
            action={(
              <Link to="/" className="rounded-md bg-primary-container px-5 py-2.5 text-sm font-semibold text-on-primary transition-opacity hover:opacity-90">
                浏览模型库
              </Link>
            )}
          />
        ) : filtered.length === 0 ? (
          <AdminEmptyState icon="search_off" title="没有匹配的分享" description="换个关键词试试。" className="min-h-[300px]" />
        ) : (
