...
function entryToPipeSpec(entry: ThreadSizeEntry): PipeSpec {
  ...
  return { dn: entry.primary, inch: entry.secondary.replace(/"/g, ""), odMm: firstNumber(entry.meta), commonUse: entry.note };
}
...
const TABLE_SCROLL = "min-h-0 flex-1 max-w-full overflow-auto border-y border-outline-variant/10 overscroll-contain [-webkit-overflow-scrolling:touch] md:border-x";
const TABLE_BASE = "min-w-full border-separate border-spacing-0 text-left text-[13px] md:text-sm [&_td]:whitespace-nowrap [&_th]:whitespace-nowrap [&_td:not(:last-child)]:border-r [&_td:not(:last-child)]:border-outline-variant/8 [&_th:not(:last-child)]:border-r [&_th:not(:last-child)]:border-outline-variant/10";
const TABLE_HEAD = "text-on-surface";
const TABLE_CARD = "flex h-full min-h-0 flex-col overflow-hidden bg-transparent";
const TABLE_TH = "sticky top-0 z-20 bg-surface-container-low px-4 py-3 text-xs font-bold tracking-wide text-on-surface shadow-[0_1px_0_rgba(0,0,0,0.08)] md:text-[13px]";
const TABLE_FIRST_TH = "sticky left-0 top-0 z-30 bg-surface-container-low px-4 py-3 text-xs font-bold tracking-wide text-on-surface shadow-[1px_0_0_rgba(0,0,0,0.08),0_1px_0_rgba(0,0,0,0.08)] md:text-[13px]";
...
function isMetricThreadFamily(family: ThreadFamily) {
  return family === "metric" || family === "metricH" || family === "metricA" || family === "metricC" || family === "metricD";
}
...
function parseMeasurementQuery(value: string): MeasurementQuery {
  ...
  const quickMeasurementWord = /(外螺纹|外牙|公螺纹|公牙|外径|大径|内螺纹|内牙|母螺纹|母牙|内孔|孔径|内径|底孔|钻孔|攻牙|小径|牙距|螺距|牙\/英寸|牙每英寸|每英寸|毫米|mm|tpi|\bod\b|\bid\b|牙数|牙)/i.test(text);
  ...
  if ((commonPipeNameAliases(text) || /^dn\d+/i.test(normalized) || /[寸分]/.test(text)) && !quickMeasurementWord) return { hasMeasurement: false };
  ...
  const hasMeasurementWord = /(外螺纹|外牙|公螺纹|公牙|外径|大径|内螺纹|内牙|母螺纹|母牙|内孔|孔径|内径|底孔|钻孔|攻牙|小径|牙距|螺距|牙\/英寸|牙每英寸|每英寸|毫米|mm|tpi|\bod\b|\bid\b|牙数|牙)/i.test(text);
  const looksLikeMeasurementOnly = /^[\s\d.,，;；:/\\+\-毫米牙距螺距外径大径内径底孔小径内孔孔径钻孔攻牙牙英寸mtpiodid]+$/i.test(normalized);
  ...
  const innerMatch = text.match(/(?:内螺纹|内牙|母螺纹|母牙|内孔|孔径|内径|底孔|钻孔|攻牙|小径|\bid\b)\s*[:：=]?\s*(\d+(?:\.\d+)?)/i);
  const tpiMatch = text.match(/(?:tpi|牙数|牙\/英寸|牙每英寸|每英寸)\s*[:：=]?\s*(\d+(?:\.\d+)?)|(\d+(?:\.\d+)?)\s*(?:牙\/英寸|牙每英寸|每英寸|tpi|牙\b)/i);
...
function compareFittingCodeAsc(a: FittingSpec, b: FittingSpec) {
  return Number(a.code) - Number(b.code)
    || a.form.localeCompare(b.form, "zh-Hans-CN");
}
...
function detectToolTab(value: string, fallback: ToolTab): ToolTab {
  ...
  if (/^\d{5}(?:-\d{2}){0,2}$/.test(q) || q.includes("扣压") || q.includes("插芯") || q.includes("直咀") || q.includes("弯咀") || q.includes("美制c型") || q.includes("公制c型") || q.includes("公制d型") || q.includes("公制h型")) return "fitting";
  ...
  if (aliases.some((alias) => /^(g|r|rc|pt|zg|npt|jic|m)\d/.test(alias)) || q.includes("h型") || q.includes("a型") || q.includes("c型") || q.includes("d型") || q.includes("unf") || q.includes("螺纹")) return "thread";
...
function familyFromQuery(value: string): "all" | ThreadFamily {
  ...
  if (aliases.some((alias) => alias.startsWith("r") || alias.startsWith("rc") || alias.startsWith("pt") || alias.startsWith("zg"))) return "r";
...
export default function ThreadSizeToolPage() {
  ...
  const { data: publicData, error: publicError, isLoading: publicLoading, mutate: mutatePublicData } = useSWR("thread-size-public", () => threadSizeApi.listPublic());
  ...
  const pipeItems = useMemo(
    () => dbEntries.filter((item) => item.kind === "pipe").map(entryToPipeSpec),
    [dbEntries],
  );
  const hoseItems = useMemo(
    () => dbEntries.filter((item) => item.kind === "hose").map(entryToHoseSpec),
    [dbEntries],
  );
  ...
  const hasTechnicalData = threadItems.length > 0 || pipeItems.length > 0 || hoseItems.length > 0 || fittingItems.length > 0;
  const adminRows = useMemo<AdminDataRow[]>(() => {
    return adminEntries.map(entryToAdminRow);
  }, [adminEntries]);
  const adminCounts = useMemo(() => CATEGORY_FILTERS.reduce<Record<string, number>>((acc, tab) => {
    acc[tab.key] = adminRows.filter((row) => matchesAdminFilter(row, tab.key)).length;
    return acc;
  }, {}), [adminRows]);
  const visibleAdminRows = useMemo(
    () => adminRows.filter((row) => matchesAdminFilter(row, managementCategory) && includesAdminQuery(row, managementQuery)),
  ...
  const matchedThreads = useMemo(() => {
    ...
    const activeFamily = measurementQuery.family && measurementQuery.family !== "all"
    ...
    return threadItems
      ...
      .slice(0, 8);
  }, [family, measurementQuery.family, measurementQuery.inner, measurementQuery.outer, measurementQuery.pitchMm, threadItems]);

  const filteredPipes = useMemo(() => {
    const aliases = queryAliases(query);
    const items = aliases.length ? pipeItems.filter((item) => includesAnyAlias(`${item.dn}${item.inch}${item.odMm}${item.commonUse}`, aliases)) : pipeItems;
  ...
  const filteredFittings = useMemo(() => {
    ...
    const items = aliases.length
      ? fittingItems.filter((item) => {
        ...
        return includesAnyAlias(`${item.code}${item.category}${item.form}${item.threadType}${item.threadCodes}${item.threadSpecs}${item.insertCodes}${item.hoseSizes}${item.seal}${item.remark}`, aliases);
      })
      : fittingItems;
    return rankedItems(
      items,
      query,
      "fitting",
      (item) => `${item.code}${item.category}${item.form}${item.threadType}${item.threadCodes}${item.threadSpecs}${item.insertCodes}${item.hoseSizes}${item.seal}${item.remark}`,
  ...
  const showDatabaseEmpty = !showGuide && !showMeasurementResults && !publicError && !publicLoading && !hasTechnicalData;
  ...
  const categoryKey = showGuide
    ...
    : visibleTab === "pipe"
      ...
      : visibleTab === "fitting"
        ? "fitting"
      : `hose:${hoseKind}`;
  const selectedCategoryKey = categoryKey === "guide"
    ? "guide"
  ...
  return (
    <AdminPageShell mobileMainClassName="overflow-hidden" mobileContentClassName="h-full min-h-0 !pb-[4.5rem]">
        ...
        toolbar={(
          <div className="grid min-w-0 items-center gap-3 md:grid-cols-[15rem_minmax(0,1fr)] lg:grid-cols-[16rem_minmax(0,1fr)]">
            <label className="relative block w-full">
              <Icon name="search" size={16} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-on-surface-variant/50" />
              <input
                value={query}
                onChange={(e) => {
              ...
              />
              {query && (
                <button onClick={clearSearch} className="absolute right-1.5 top-1/2 -translate-y-1/2 rounded-md px-2 py-1 text-xs font-medium text-on-surface-variant transition-colors hover:bg-surface-container-high hover:text-on-surface">
                  清空
                </button>
              )}
            </label>
      ...
      >
        <AdminContentPanel scroll className="h-full flex min-h-0 flex-col overflow-hidden rounded-none border-0 bg-transparent">

          {/* ── Results ── */}
          <div key={`${showGuide ? "guide" : visibleTab}:${showMeasurementResults ? "measurement" : showDataError ? "error" : showDatabaseEmpty ? "database-empty" : showNoResults ? "empty" : "results"}`} className="admin-tab-panel min-h-0 flex-1 overflow-hidden">
            {showGuide && (
              <section className="h-full overflow-y-auto overflow-x-hidden md:overflow-hidden">
                <div className="grid min-h-full gap-2 md:h-full md:min-h-0 md:grid-rows-[auto_minmax(0,1fr)] md:gap-3">
                  <div className="rounded-xl border border-outline-variant/12 bg-surface-container-low p-3 md:p-4">
                      <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-primary-container/70">Quick Lookup</p>
                  ...
                  <div className="grid min-h-0 grid-cols-1 gap-1.5 min-[390px]:grid-cols-2 md:grid-cols-4 md:grid-rows-1 md:gap-3 md:overflow-hidden">
                    {[
                      { title: "螺纹", desc: "公制、G、R/PT、NPT、JIC 规格，查看外径、牙距、牙型角。", icon: "hexagon", examples: ["G1/4", "G1/2", "R1/2", "NPT1/4", "M16×1.5"] },
                      { title: "俗称", desc: "几分、几寸、DN 快速换算，适合现场口头规格确认。", icon: "pipeline", examples: ["2分", "4分", "6分", "1寸", "DN25"] },
                      { title: "测量", desc: "输入卡尺外径、内螺纹小径或牙距，反推最接近螺纹。", icon: "straighten", examples: ["外径13.1", "内螺纹18.6", "牙距1.5"] },
                      { title: "管路", desc: "油管、气管、Dash、JIC 和扣压接头编号快速查。", icon: "cat_hydraulic_hose", examples: ["-06", "-08", "JIC-06", "20411", "26711"] },
                    ].map((group) => (
                      <div key={group.title} className="flex min-h-[148px] flex-col overflow-hidden rounded-xl border border-outline-variant/12 bg-surface-container-low p-2.5 min-[390px]:min-h-[168px] md:min-h-0 md:p-4">
                        <div className="flex min-h-0 items-start gap-2 md:gap-3">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="mt-1 line-clamp-2 text-[11px] leading-4 text-on-surface-variant md:mt-1.5 md:text-sm md:leading-5">{group.desc}</p>
                          </div>
                        </div>
                ...
                <div className={TABLE_CARD}>
                    <div className={TABLE_HEADER}>
                      ...
                      <div className="flex shrink-0 flex-wrap justify-end gap-1">
                        ...
                        {measurementQuery.inner && <span className="rounded bg-surface-container-low px-1.5 py-0.5 text-[10px] text-on-surface-variant">内径/小径 {measurementQuery.inner}mm</span>}
                        ...
                        {measurementQuery.family && measurementQuery.family !== "all" && <span className="rounded bg-primary-container/10 px-1.5 py-0.5 text-[10px] text-primary-container">{CATEGORY_FILTERS.find((item) => item.key === `thread:${measurementQuery.family}`)?.label || measurementQuery.family}</span>}
                      </div>
                    </div>
                    ...
                    <table className={`${TABLE_BASE} min-w-[1320px]`}>
                      ...
                      <tbody className="divide-y divide-outline-variant/10">
                        {filteredFittings.map((item) => (
                          <tr key={item.code} onClick={() => applyResultAsSearch(item.code)} className="cursor-pointer text-on-surface transition-colors hover:bg-surface-container-high/30 active:bg-primary-container/10">
                            <td className={`${TABLE_FIRST_TD} min-w-36`}>
                              <span className="rounded-md bg-primary-container/10 px-2 py-1 font-semibold text-primary-container">{item.code}</span>
                            </td>
                            ...
                            <td className={`${TABLE_TD} min-w-52 max-w-[320px] leading-6 [white-space:normal]`}>{item.insertCodes}</td>
                            <td className={TABLE_TD}>{item.seal}</td>
            ...
            {showNoResults && (
              <section className="flex h-full items-center justify-center overflow-y-auto px-4 py-10 text-center">
                <div className="max-w-sm">
                  ...
                  <p className="mt-2 text-xs leading-6 text-on-surface-variant/70">换成规格、俗称、型号片段或测量值再试<br />例如 G1/2、4分、DN25、-06、20.9 14牙</p>
                </div>
              </section>
            )}
          </div>
      ...
      {managementOpen && (
        <div className="fixed inset-0 z-[320] bg-black/50 p-0 sm:flex sm:items-center sm:justify-center sm:p-4" onClick={() => setManagementOpen(false)}>
          <div
          ...
          >
            ...
            <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
              <div className="flex shrink-0 items-center justify-between gap-3 px-4 py-3">
                <div className="min-w-0">
                  <p className="text-sm font-bold text-on-surface">{CATEGORY_FILTERS.find((tab) => tab.key === managementCategory)?.label}</p>
                  <p className="mt-0.5 text-xs text-on-surface-variant">
                    {adminRows.length ? "当前读取数据库数据，修改后前台立即生效。" : "数据库暂无规格数据，请新增记录或通过数据库导入。"}
                  </p>
                </div>
      ...
      {editingEntry && (
        <div className="fixed inset-0 z-[340] flex items-center justify-center bg-black/55 p-3" onClick={() => setEditingEntry(null)}>
          <div
          ...
          >
            <div className="flex shrink-0 items-center justify-between gap-3 border-b border-outline-variant/10 px-4 py-3">
              <div className="min-w-0">
                <h2 className="text-base font-bold text-on-surface">{editingEntry === "new" ? "新增规格数据" : "编辑规格数据"}</h2>
              ...
              <div className="grid gap-3 md:grid-cols-[1fr_auto] md:items-end">
                <label className="block">
                  ...
                  <input
                    ...
                    onChange={(event) => setEntryDraft((prev) => ({ ...prev, sortOrder: Number(event.target.value) || 0 }))}
                    className="mt-1 h-10 w-full rounded-lg border border-outline-variant/20 bg-surface-container-lowest px-3 text-sm text-on-surface outline-none"
                  />
                </label>
            ...
            <div className="flex shrink-0 justify-end gap-2 border-t border-outline-variant/10 px-4 py-3">
              <button type="button" onClick={() => setEditingEntry(null)} className="h-9 rounded-lg border border-outline-variant/20 px-4 text-sm font-bold text-on-surface-variant">
                取消
              </button>
              <button type="button" onClick={() => void saveEntryDraft()} className="h-9 rounded-lg bg-primary-container px-4 text-sm font-bold text-on-primary">
                保存
              </button>
            </div>
          </div>
