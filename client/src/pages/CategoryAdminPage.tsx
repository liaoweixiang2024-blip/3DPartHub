      ...
      >
        <span
          ...
          className={`hidden h-8 w-7 shrink-0 select-none items-center justify-center rounded-lg text-on-surface-variant/45 sm:inline-flex ${
            dragDisabled ? "cursor-not-allowed opacity-30" : "cursor-grab hover:bg-surface-container-high hover:text-on-surface-variant active:cursor-grabbing"
          }`}
        ...
        <div className="hidden sm:block">
          {hasChildren ? (
            ...
            >
              <Icon name="expand_more" size={18} className={`transition-transform ${collapsed ? "-rotate-90" : "rotate-0"}`} />
            </button>
          ) : (
            <span className="block h-8 w-8" />
          )}
        </div>
        <span className={`hidden h-10 w-10 shrink-0 place-items-center rounded-xl sm:grid ${isChild ? "bg-surface-container-high text-on-surface-variant" : "bg-primary-container/10 text-primary-container"}`}>
        ...
        <div className="flex min-w-0 items-center gap-2" style={{ paddingLeft: depth * (depth > 0 ? 12 : 0) }}>
          ...
          <span className={`grid h-9 w-9 shrink-0 place-items-center rounded-lg sm:hidden ${isChild ? "bg-surface-container-high text-on-surface-variant" : "bg-primary-container/10 text-primary-container"}`}>
            <Icon name={cat.icon || (hasChildren ? "folder" : "view_in_ar")} size={18} />
          </span>
          <div className="min-w-0">
            <div className="flex min-w-0 items-center gap-2">
              <span className="truncate text-sm font-semibold text-on-surface sm:text-[15px]">{cat.name}</span>
              {isChild && <span className="hidden rounded-full bg-surface-container-high px-2 py-1 text-[10px] text-on-surface-variant sm:inline">子分类</span>}
            </div>
        ...
        <div className="flex items-center justify-end gap-1">
          {!isChild && (
            <button onClick={() => onAddChild(cat.id)} className="hidden rounded-lg px-2.5 py-2 text-xs font-medium text-primary-container hover:bg-primary-container/10 sm:inline-flex sm:items-center sm:gap-1" aria-label="添加子分类">
              <Icon name="add" size={14} /> 子类
            </button>
          )}
          <button onClick={() => onEdit(cat)} className="grid h-8 w-8 place-items-center rounded-lg text-on-surface-variant transition-colors hover:bg-primary-container/10 hover:text-primary-container" aria-label="编辑">
            <Icon name="edit" size={15} />
          </button>
          <button onClick={() => onDelete(cat)} className="grid h-8 w-8 place-items-center rounded-lg text-on-surface-variant transition-colors hover:bg-error-container/10 hover:text-error" aria-label="删除">
            <Icon name="delete" size={15} />
          </button>
        </div>
      </div>
      <AnimatePresence initial={false}>
        {hasChildren && !collapsed && cat.children?.map((child) => (
          <motion.div
            key={child.id}
    ...
    >
      ...
      >
        <div className="flex items-start justify-between gap-3 border-b border-outline-variant/10 px-5 py-4">
          <div>
            ...
            <p className="mt-1 text-xs text-on-surface-variant">
              分类会用于模型库筛选、上传归类和模型详情页展示。
            </p>
          </div>
          <button onClick={onClose} className="grid h-8 w-8 shrink-0 place-items-center rounded-full text-on-surface-variant hover:bg-surface-container-high hover:text-on-surface">
        ...
        <div className="space-y-5 px-5 py-5">
          <div className="grid gap-3 sm:grid-cols-[72px_1fr] sm:items-start">
            ...
            <input
              ...
              autoFocus
            />
              <p className="mt-1.5 text-[11px] text-on-surface-variant/70">建议使用产品族名称，例如“不锈钢接头”“气动阀门”。</p>
            </div>
          </div>
          ...
          <div>
            <div className="mb-2 flex items-center justify-between">
              ...
              <span className="rounded-full bg-surface-container-high px-2 py-1 text-[10px] text-on-surface-variant">{icon}</span>
            </div>
            <div className="grid grid-cols-5 gap-2 sm:grid-cols-7">
              {iconOptions.map((ic) => (
                <button
                  ...
                  title={ic}
                >
                  <Icon name={ic} size={20} className={icon === ic ? "text-primary-container" : "text-on-surface-variant group-hover:text-on-surface"} />
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 gap-2 border-t border-outline-variant/10 bg-surface-container-low px-5 py-4 sm:flex sm:justify-end">
          <button onClick={onClose} className="rounded-xl bg-surface-container-high/50 px-4 py-2.5 text-sm text-on-surface-variant hover:bg-surface-container-high hover:text-on-surface sm:rounded-lg">取消</button>
          <button
            onClick={handleSave}
...
function Content() {
  ...
  const allRootChildrenCollapsed = rootIdsWithChildren.length > 0 && rootIdsWithChildren.every((id) => collapsedIds.has(id));
              ...
              {tree.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-16 gap-3">
                  ...
                  <div className="text-center">
                    ...
                    <p className="mt-1 text-xs text-on-surface-variant">添加第一个分类后，上传和模型库就可以按分类筛选。</p>
                  </div>
                  <button onClick={handleAdd} className="rounded-xl bg-primary-container px-4 py-2 text-sm font-bold text-on-primary hover:opacity-90">添加分类</button>
                </div>
                ...
                <div className="flex flex-col items-center justify-center py-14 gap-2">
                  ...
                  <button onClick={() => setQuery("")} className="text-sm text-primary-container hover:underline">清空搜索</button>
                </div>
              )}
            </div>
          </AdminContentPanel>
            ...
            >
              <div className="mb-4 flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm leading-relaxed text-on-surface-variant">删除后不可恢复，如果分类下有关联模型，可能会影响模型筛选展示。</p>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <button onClick={() => setDeleteConfirm(null)} className="rounded-xl bg-surface-container-high/50 px-4 py-2.5 text-sm text-on-surface-variant hover:bg-surface-container-high hover:text-on-surface">取消</button>
                <button onClick={() => handleDelete(deleteConfirm)} className="rounded-xl bg-error px-4 py-2.5 text-sm font-bold text-on-error hover:opacity-90">确认删除</button>
              </div>
            </motion.div>
          </motion.div>
        )}
