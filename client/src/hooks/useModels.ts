...
export function useInfiniteModels(
  params?: PaginationParams & { category?: string; categoryId?: string; search?: string; format?: string; sort?: string },
  initialSize = 1
) {
  const pageSize = params?.pageSize || 50;
  const search = params?.search || "";
  ...
  const { data, error, isLoading, isValidating, mutate, setSize, size } = useSWRInfinite<PaginatedResponse<ServerModelListItem>>(
    getKey,
    (key: string) => {
  ...
  return {
    data: firstPage
      ? { ...firstPage, items, page: pages.length, totalPages }
      : undefined,
    error,
