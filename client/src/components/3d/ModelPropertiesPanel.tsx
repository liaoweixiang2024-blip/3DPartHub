...
function PropertyRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between gap-3 border-b border-outline-variant/10 py-2 last:border-b-0">
      ...
      <span className="min-w-0 truncate text-right font-mono text-xs text-on-surface" title={value}>{value}</span>
    </div>
  );
}
...
export default function ModelPropertiesPanel({
  ...
  const fallbackVertexCount = useMemo(
    () => parts.reduce((sum, part) => sum + part.vertexCount, 0),
    [parts]
  );
  const fallbackFaceCount = useMemo(
    () => parts.reduce((sum, part) => sum + part.triangleCount, 0),
    [parts]
  );
    ...
    >
      <div className="flex items-start justify-between gap-3 border-b border-outline-variant/15 px-3 py-3">
        <div className="min-w-0">
          ...
          <p className="mt-1 truncate text-[11px] text-on-surface-variant" title={modelName || previewMeta?.sourceName || ""}>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-3" style={{ WebkitOverflowScrolling: "touch" }}>
        <div className="grid grid-cols-3 gap-2">
          ...
          <div className="rounded-sm bg-surface-container-low p-2">
            ...
            <span className="font-mono text-sm text-on-surface">{formatCount(totals?.vertexCount ?? fallbackVertexCount)}</span>
          </div>
          <div className="rounded-sm bg-surface-container-low p-2">
            <span className="block text-[10px] text-on-surface-variant">面</span>
            <span className="font-mono text-sm text-on-surface">{formatCount(totals?.faceCount ?? fallbackFaceCount)}</span>
          </div>
        </div>
