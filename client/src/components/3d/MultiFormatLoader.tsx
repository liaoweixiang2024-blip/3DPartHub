...
function createMaterial(
  preset: RenderMaterialPresetKey,
...
function syncEdgeTransforms(root: THREE.Object3D) {
  root.traverse((child) => {
    ...
    if (overlay.parent) {
      ...
      overlay.matrix.copy(overlay.parent.matrixWorld).invert().multiply(child.matrixWorld);
    } else {
...
function meshPath(mesh: THREE.Mesh) {
  ...
  while (current) {
    if (current.name && !current.name.startsWith('__')) names.unshift(normalizeCadLabel(current.name, current.name));
    current = current.parent;
  }
  return names.join(' / ');
}
...
function applyExplodeState(
  ...
  boundsDetail: ModelBoundsDetail | null,
  viewMode: ViewMode,
  explodeAmount = 1,
) {
...
function measurementPointFromEvent(event: any, snapMode: MeasurementSnapMode = 'surface'): MeasurementPoint {
  ...
  if (snapMode === 'vertex') {
    const closestVertex = triangle.reduce(
      (best, candidate) =>
...
function getClipPlaneValues(direction: 'x' | 'y' | 'z', position: number, inverted?: boolean) {
  ...
  return {
    normal: new THREE.Vector3(direction === 'x' ? sign : 0, direction === 'y' ? sign : 0, direction === 'z' ? sign : 0),
...
function CadModel({
    ...
    return () => {
      ...
      cadGroup.traverse((child) => {
        if (child instanceof THREE.Mesh) {
          ...
          const edge = child.userData._edgeOverlay as THREE.LineSegments | undefined;
          if (edge) {
  ...
  // Dispose base material when preset changes
  useEffect(() => {
    return () => {
  ...
  const edgesVisible = showEdges;
  const edgeViewMode = viewMode;
  useFrame(() => {
    if (!cadGroup) return;
    cadGroup.traverse((child) => {
      if (child instanceof THREE.Mesh && !child.name.startsWith('__cad_')) {
    ...
    if (edgesVisible && edgeViewMode !== 'wireframe') {
      syncEdgeTransforms(cadGroup);
    }
  });
  ...
  return (
    <group onClick={handlePartClick}>
      ...
      {showDimensions && centeredBox && (
        <group>
          <DimensionLine
            start={new THREE.Vector3(centeredBox.min.x, centeredBox.min.y, centeredBox.min.z)}
            end={new THREE.Vector3(centeredBox.max.x, centeredBox.min.y, centeredBox.min.z)}
            label={`${(centeredBox.max.x - centeredBox.min.x).toFixed(1)} mm`}
          />
          <DimensionLine
            start={new THREE.Vector3(centeredBox.min.x, centeredBox.min.y, centeredBox.min.z)}
            end={new THREE.Vector3(centeredBox.min.x, centeredBox.max.y, centeredBox.min.z)}
            label={`${(centeredBox.max.y - centeredBox.min.y).toFixed(1)} mm`}
          />
          <DimensionLine
            start={new THREE.Vector3(centeredBox.min.x, centeredBox.min.y, centeredBox.min.z)}
            end={new THREE.Vector3(centeredBox.min.x, centeredBox.min.y, centeredBox.max.z)}
            label={`${(centeredBox.max.z - centeredBox.min.z).toFixed(1)} mm`}
          />
        </group>
      )}
...
function GltfModel({
  ...
  const edgesVisible2 = showEdges;
  const edgeViewMode2 = viewMode;
  const edgeScene = clonedScene;
  useFrame(() => {
    if (!edgeScene) return;
    ...
    if (edgesVisible2 && edgeViewMode2 !== 'wireframe') {
      syncEdgeTransforms(edgeScene);
    }
  });
...
function ClipPlaneOverlay({
  ...
  const {
    ...
    const normalEndPosition = markerPosition
      .clone()
      .add(values.normal.clone().normalize().multiplyScalar(normalLength));
...
function DimensionLine({ start, end, label }: { start: THREE.Vector3; end: THREE.Vector3; label: string }) {
  ...
  const connA = useMemo(
    () => new Float32Array([start.x, start.y, start.z, oStart.x, oStart.y, oStart.z]),
    [start, oStart],
  );
  const connB = useMemo(() => new Float32Array([end.x, end.y, end.z, oEnd.x, oEnd.y, oEnd.z]), [end, oEnd]);
  const mainLine = useMemo(
    () => new Float32Array([oStart.x, oStart.y, oStart.z, oEnd.x, oEnd.y, oEnd.z]),
    [oStart, oEnd],
  );
    ...
    <group renderOrder={muted ? 18 : 20}>
      ...
      {vectors.map((point, index) => (
        <mesh key={index} position={point} renderOrder={muted ? 19 : 21}>
          ...
          <meshBasicMaterial
            color={index === 0 ? startColor : endColor}
            transparent={muted}
      ...
      <Html position={[mid.x, mid.y, mid.z]} center>
        <div
          className={`pointer-events-none select-none rounded-sm border px-2 py-1 text-[11px] font-mono shadow-lg ${muted ? 'border-slate-300/35 bg-black/50 text-slate-100' : 'border-cyan-300/50 bg-black/75 text-cyan-100'}`}
        >
          {label}
        </div>
      </Html>
...
function MeasurementOverlay({
  ...
  return (
    <group>
      ...
      {mode !== 'bounds' && <SingleMeasurementOverlay mode={activeMode} points={points} maxDim={maxDim} />}
    </group>
  );
}
