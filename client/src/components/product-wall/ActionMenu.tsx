...
const btnBase = "product-wall-action inline-flex h-8 items-center justify-center gap-1.5 rounded-md text-sm font-medium transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-container/35 disabled:cursor-not-allowed disabled:opacity-45";
...
export default function ProductWallActionMenu({
  ...
  const adminItems = isAdmin && (
    <>
      <MenuItem icon={wallEditMode ? "close" : "edit"} label={wallEditMode ? "退出编辑" : "编辑"} disabled={!selectableVisibleItems.length} onClick={onToggleEditMode} />
      <MenuItem icon={selectionMode ? "close" : "delete"} label={selectionMode ? "退出批量" : "批量删除"} disabled={!selectableVisibleItems.length} onClick={onToggleSelectionMode} />
      {selectionMode && (
        <>
          <MenuDivider />
          <MenuItem icon="check" label="全选当前" disabled={!selectableVisibleItems.length} onClick={onSelectAll} />
          <MenuItem icon="delete" label={`删除已选${selectedCount ? ` (${selectedCount})` : ""}`} disabled={!selectedCount} danger onClick={onDeleteSelected} />
          <MenuItem icon="close" label="取消选择" onClick={onClearSelection} />
        </>
      )}
      <MenuDivider />
      <MenuItem icon="settings" label="图片管理" onClick={onOpenManagement} />
      <MenuItem icon="folder" label="上传文件夹" disabled={uploadDisabled} onClick={() => { setManageMenuOpen(false); folderInputRef.current?.click(); }} />
    </>
  );
  ...
  if (variant === "mobile") {
    return (
      <div className="product-wall-action relative flex items-center gap-1 md:hidden" onClick={(e) => e.stopPropagation()}>
        <button
          type="button"
