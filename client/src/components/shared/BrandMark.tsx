...
export default function BrandMark({ size = "nav", centered = false, className = "", titleSuffix }: BrandMarkProps) {
  ...
  const title = (
    <span className={`${cfg.title} min-w-0 truncate font-headline font-bold leading-none tracking-tight text-on-surface`}>
      {siteTitle}
    </span>
  );
  ...
  const image = (src: string, kind: "icon" | "logo") => (
    <span
      ...
      aria-hidden="true"
    >
      <SafeImage src={src} alt="" className="block max-h-full max-w-full object-contain align-middle" fallbackIcon="view_in_ar" />
    </span>
  );
  ...
  return (
    <span className={`flex min-w-0 items-center ${centered ? "justify-center" : ""} ${cfg.gap} leading-none ${className}`}>
      {content}
      {titleSuffix}
    </span>
  );
