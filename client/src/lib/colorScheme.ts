...
export function applyColorScheme(
  ...
  if (scheme && scheme !== 'custom' && COLOR_PRESETS[scheme]) {
    ...
    try { darkVars = JSON.parse(customDark || '{}'); } catch { darkVars = {}; }
    try { lightVars = JSON.parse(customLight || '{}'); } catch { lightVars = {}; }
  }
...
export function generatePaletteFromPrimary(primaryHex: string): {
  ...
  const tone = (l: number, satMul = 1, hueShift = 0) =>
    hslToHex((h + hueShift + 360) % 360, Math.round(s * satMul), l);
