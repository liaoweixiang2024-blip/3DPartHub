...
function cleanFileName(value: string) {
  return value.replace(/[\\/:*?"<>|]+/g, "-").replace(/\s+/g, " ").trim() || "kit-list";
}

export function getKitListTitle(
...
export function formatKitList(product: SelectionProduct, components: SelectionComponent[], title = getKitListTitle(null, product)) {
  const lines = [
...
export function downloadKitList(product: SelectionProduct, components: SelectionComponent[], title = getKitListTitle(null, product)) {
