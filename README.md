...
脚本会自动完成：复用已有 Docker 或安装 Docker → 修复 Docker apt 源/GPG Key → 下载 docker-compose.yml → 生成随机密钥 → 写入生产访问来源 → 按服务器内存自动分配资源 → 检查 3780 端口 → 拉取最新镜像并启动 → 运行部署自检。
...
自检脚本不会重启服务或修改配置；除备份目录写入并删除一个隐藏探针文件外，只读取运行状态。它会检查 Docker/Compose、Compose 持久化挂载、Compose/运行容器日志轮转、API 停止宽限期、Compose 内部网络、Compose/运行容器端口暴露、运行容器重启策略、Redis healthcheck 是否使用 `REDISCLI_AUTH` 认证 ping、实际容器挂载、运行容器关键环境是否与 `.env` 一致、API 主进程是否非 root 运行、`.env` 密钥、`ALLOWED_ORIGINS` 和文件权限、备份签名/加密密钥、`IMAGE_TAG` 与运行镜像标签、运行镜像来源、运行版本、容器健康、容器镜像、OOMKilled/重启次数、数据库密码、数据库迁移状态、Redis、端口、Web 敏感路径和 X-Accel 内部下载路径、健康/就绪/存活接口、管理健康接口访问控制、API/Web 安全响应头、Web 首页入口、Web 前端静态资源、宿主机备份目录可写性、备份恢复演练证据、部署目录磁盘和 inode、Docker 数据目录磁盘和 inode、备份目录磁盘和 inode、API 数据卷容量、服务器内存和资源配置预算、内存以及 API/Web 常见日志错误，并可保存带生成时间、主机和系统信息的 `deploy-health-report.txt` 方便人工排障，也可同时保存 `deploy-health-report.json` 方便自动归档和比对。核心容器未配置 healthcheck、最近发生过 OOMKilled、重启次数较高、API 停止宽限期缺失或过短、备份恢复演练证据缺失或过期、API 主进程以 root 运行、`.env` 权限过宽、资源配置超过当前内存档位、运行容器关键环境与 `.env` 不一致、api/web 运行镜像标签与 `IMAGE_TAG` 不一致、运行镜像来源异常、管理健康接口未受保护、运行版本接口异常、数据库迁移状态异常、API 数据卷容量不足、存活接口失败、API/Web 安全响应头缺失、Web 首页入口无法返回前端 HTML、Web 前端静态资源无法加载、Web 敏感路径或 X-Accel 内部下载路径暴露、API/Web 日志包含常见入口或启动错误、关键持久化挂载缺失、运行容器挂载仍是旧配置、Compose/运行容器日志轮转缺失、私有服务端口暴露、运行容器重启策略未生效，或 Compose 服务未设置 `restart: unless-stopped`，都会记为警告或失败，生产验收默认不接受。需要把警告也当成失败时可执行 `sh deploy-health-check.sh --strict`。如果生产环境使用了非默认文件名或目录，必须显式传入，例如 `sh deploy-health-check.sh --compose-file /opt/3dparthub/docker-compose.yml --env-file /opt/3dparthub/.env --report deploy-health-report.txt --json deploy-health-report.json`。
...
一键部署会在服务启动后自动运行同一套自检，并在 `/opt/3dparthub/deploy-health-report.txt` 和 `/opt/3dparthub/deploy-health-report.json` 留下最新报告；如果服务启动失败、初始健康检查未通过或自检失败，部署脚本会自动尝试生成 `deploy-evidence-failed-YYYYMMDD-HHMMSS.tar.gz` 和同名 `.sha256`，方便直接回传排查。临时排障需要跳过时可设置 `SKIP_DEPLOY_CHECK=1`。
...
完整验收口径见 [生产部署健康验收清单](docs/部署健康验收清单.md)。

修改一键部署、自检或证据包脚本后，可先执行部署专项验收：
...
> **命令提示：** 以下统一用 `docker-compose`（连字符，官方独立二进制；宝塔 / 二进制安装的服务器开箱即用）。如果你的系统只有插件形式 `docker compose`（空格），两者完全等价，把命令里的 `docker-compose` 换成 `docker compose` 即可。
...
> 以下命令在部署目录 `/opt/3dparthub` 下执行。统一用 `docker-compose`（连字符）；等价于 `docker compose`（空格，插件形式），按你的系统二选一即可。
