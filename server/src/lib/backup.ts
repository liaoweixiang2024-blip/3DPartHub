...
async function runRestore(job: RestoreJob, backupId: string) {
  ...
  try {
    ...
    if (existsSync(sqlPath)) {
      ...
      // Strip \restrict command generated by pg_dump v18+ before importing
      // \restrict blocks backslash commands including \. (COPY terminator), breaking data import
      execSync(`sed '/^\\\\restrict/d' "${sqlPath}" | psql "${DB_URL_CLEAN}" -v ON_ERROR_STOP=0`, { stdio: "pipe", timeout: 300_000 });
