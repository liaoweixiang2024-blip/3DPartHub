...
export function createAdminSharesRouter() {
  ...
  router.get("/api/admin/shares", authMiddleware, async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      const selectionItems: AdminShareItem[] = selectionRows.map((s) => {
        const selectionName = selectionNameMap.get(s.id) || selectionCategoryMap.get(s.categorySlug) || s.categorySlug || "产品选型";
        return {
          id: `selection:${s.id}`,
          rawId: s.id,
      ...
      const items = [...modelItems, ...selectionItems]
  ...
  router.post("/api/admin/shares/batch-delete", authMiddleware, async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      const [modelResult, selectionResult] = await Promise.all([
        modelIds.length ? prisma.shareLink.deleteMany({ where: { id: { in: modelIds } } }) : Promise.resolve({ count: 0 }),
        selectionIds.length ? prisma.selectionShare.deleteMany({ where: { id: { in: selectionIds } } }) : Promise.resolve({ count: 0 }),
      ]);
