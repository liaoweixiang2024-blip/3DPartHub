    ...
    .then(({ uploadPolicy }) => {
      ...
      upload(req, res, (err) => {
        ...
        if (uploadError.code === 'LIMIT_FILE_SIZE') {
          res
            .status(400)
            .json({ detail: `图片不能超过 ${Math.round(optionImageMaxBytes(uploadPolicy) / 1024 / 1024)}MB` });
          return;
        }
...
export function createSelectionOptionImagesRouter() {
  ...
  // Upload option image
  router.post(
    '/api/admin/selections/option-image',
    authMiddleware,
    requireAdmin,
    optionImageUpload,
  ...
  router.post(
    '/api/admin/selections/product-asset',
    authMiddleware,
    requireAdmin,
    productAssetUpload,
    async (req: AuthRequest, res: Response) => {
  ...
  router.post(
    '/api/admin/selections/option-image-from-url',
    authMiddleware,
    async (req: AuthRequest, res: Response) => {
      if (!adminOnly(req, res)) return;
      try {
