...
export function createConversionQueueRouter() {
  ...
  router.get(
    ...
    async (req: AuthRequest, res: Response) => {
      try {
        ...
        const completedModelCount = await prisma.model
          .count({ where: { status: 'completed' } })
          .catch(() => counts.completed || 0);
        const queueCounts = {
        ...
        const jobStates: JobType[] =
          stateFilter === 'all' ? ['active', 'waiting', 'delayed', 'failed', 'completed'] : [stateFilter];
  ...
  router.post(
    '/api/tasks/conversion-queue/retry-failed',
    authMiddleware,
    requireRole('ADMIN'),
    async (req: AuthRequest, res: Response) => {
      try {
        ...
        for (const job of jobs) {
          ...
          try {
            ...
            if (prisma && job.data?.modelId) {
              await prisma.model
                .update({ where: { id: job.data.modelId }, data: { status: 'queued' } })
                .catch(() => {});
            }
            retried++;
  ...
  router.post(
    '/api/tasks/conversion-queue/cancel-rebuilds',
    authMiddleware,
    requireRole('ADMIN'),
    async (req: AuthRequest, res: Response) => {
      try {
  ...
  router.post(
    '/api/tasks/conversion-queue/clean',
    authMiddleware,
    requireRole('ADMIN'),
    async (req: AuthRequest, res: Response) => {
      try {
  ...
  router.get(
    ...
    async (req: AuthRequest, res: Response) => {
      ...
      try {
        ...
        const logResult = await conversionQueue
          .getJobLogs(String(job.id), 0, logLimit - 1, true)
          .catch(() => ({ logs: [], count: 0 }));
        const sourcePath = typeof job.data?.filePath === 'string' ? job.data.filePath : null;
        const model =
          prisma && job.data?.modelId
            ? await prisma.model
                .findUnique({
