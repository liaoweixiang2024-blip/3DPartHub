    ...
    try {
      ...
      for (const { model, diagnostic } of candidates) {
        ...
        try {
          ...
          await prisma.model.update({ where: { id: model.id }, data: { status: MODEL_STATUS.QUEUED } }).catch(() => {});
          queued++;
          items.push({ model_id: model.id, name: model.name || model.originalName, status: MODEL_STATUS.QUEUED, job_id: job.id });
        } catch (err: any) {
          failed++;
          items.push({ model_id: model.id, name: model.name || model.originalName, status: MODEL_STATUS.FAILED, reason: err?.message || "队列投递失败" });
        }
      }
